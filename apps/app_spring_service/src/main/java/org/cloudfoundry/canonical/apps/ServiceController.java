...
public class ServiceController {
	...
	public void rabbit_post(@RequestBody String body, @PathVariable String key,
			HttpServletResponse response) throws IOException {
		PrintWriter out = response.getWriter();
		AmqpTemplate amq = loadRabbit(key); 
		amq.convertAndSend(body);
		message = (String) amq.receiveAndConvert();
		out.print(body);
	}
	...
	private AmqpTemplate loadRabbit(String key) {
		RabbitTemplate template;
		try {
			template = new RabbitTemplate(connectionRabbit());
			template.setRoutingKey(key);
			template.setQueue(key);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void env(HttpServletResponse response) throws IOException {
		...
		for (AbstractServiceInfo info : (List<AbstractServiceInfo>) env
				.getServiceInfos()) {
