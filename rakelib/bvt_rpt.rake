...
class BvtEnv
  ...
  def initialize
    ...
    @vcap_dir        = File.dirname(@root_dir)
...
namespace :bvt_rpt do
  ...
  def bvt_summary(results_dir_path, git_root_path, verbose=true)
    ...
    return return_summary, total_errs, total_count, total_time_string
  end
...
END_OF_MESSAGE
  ...
  task :email_bvt_summary  do
    ...
    summary, total_errs, total_tests, total_time = bvt_summary(bvt_env.artifacts_dir, File.dirname(bvt_env.vcap_dir))
