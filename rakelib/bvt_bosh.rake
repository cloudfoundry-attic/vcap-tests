...
namespace :ci_bosh do
  ...
  task :target do
    ...
    result = `bosh target #{url}`
    if not /Target set to '.* \(#{url}\)'/.match(result)
  ...
  task :login => [:target] do
    ...
    if not /Logged in as '#{bosh_env.bosh_user}'/.match(result)
  ...
  task :deployment do
    ...
    if not /Deployment set to .*/.match(result)
  ...
  task :create_release, :fail_if_unchanged do | t, args |
    ...
    # IMPROVE_ME: when a distinct return code is available from 
    #             create release, use that.
    if result.include?("version is no different from") && fail_if_unchanged[0].downcase == "t"
      fail "bosh create release did not generate a new release; no different from predecessor"
    end
    if !result.include?("Release manifest saved in")
  ...
  task :upload_latest_release, :fail_if_exists, :needs => :login do | t, args |
    ...
    Dir.chdir(bosh_env.release_dir) do
      ...
      result = `bosh upload release #{release_yml}`
      if result.include?("has already been uploaded")
        ...
        if not /Task [\d]+: state is 'done'/.match(result)
          fail "bosh upload release failed. Result:\n#{result}"
        end
      end
    end
  ...
  task :generate_manifest do
    ...
    manifest['release'] = {'name' => config['name'], 'version' => match_data[1].to_i }
  ...
  task :delete_deployment do
    ...
    result = `bosh delete deployment #{config['name']}`
    if not /Task [\d]+: state is 'done'/.match(result)
  ...
  task :bvt_env do
    config = YAML.load_file(bosh_env.manifest_src)
    ENV['VCAP_BVT_TARGET'] = config['properties']['domain']
  ...
  task :brief_bvt_summary do
    ...
    puts "errors+failures = #{errfail_count}"
    puts "#{summary}"
  end
