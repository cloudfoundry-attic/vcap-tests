...
# Internal task:  desc "Deploy one canonical app, provision all services, and post data"
  task :deploy_one_canonical_app, :framework do |t, args|
    ...
    command = "bundle exec cucumber features/#{feature_name} --require features #{cucumber_options} --format pretty -o #{output}/TEST-#{feature_name}.log"
    puts "#{command}"
    sh("cd #{tests_dir}; #{command}")
  end

  desc "Delete all canonical app"
  task :delete_all_canonical_app do |t, args|
...
# Internal task:  desc "Verify posted data for one canonical app, and service"
  ...
  task :deploy_sinatra_canonical do
    Rake::Task['bvt_caldecott:deploy_one_canonical_app'].invoke("sinatra", "junit")
  end

  desc "Deploy and keep node canonical app running"
  task :deploy_node_canonical do
    Rake::Task['bvt_caldecott:deploy_one_canonical_app'].invoke("node", "junit")
  end

  desc "Deploy and keep spring canonical app running"
  task :deploy_spring_canonical do
    Rake::Task['bvt_caldecott:deploy_one_canonical_app'].invoke("spring", "junit")
  end

  desc "Deploy and keep rails canonical app running"
  task :deploy_rails_canonical do
    Rake::Task['bvt_caldecott:deploy_one_canonical_app'].invoke("rails", "junit")
  end
