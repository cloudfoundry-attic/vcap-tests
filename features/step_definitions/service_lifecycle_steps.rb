...
class ErrorLifecycleResponse < StandardError; end
...
Then /^I check (\w+) extension is enabled$/ do |extension|
  ...
  when "serialized"
    ...
    rescue  ErrorLifecycleResponse => e
      # ignore error
    end
  end
end
...
Then /^I delete the snapshot for (\w+) service$/ do |service|
  parse_service_id unless @service_id
...
def get_snapshots
  ...
  if easy.response_code == 501
    ...
    raise ErrorLifecycleResponse.new("code:#{easy.response_code}, body:#{easy.body_str}")
  end
